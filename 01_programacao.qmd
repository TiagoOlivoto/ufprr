...
```{r, echo = TRUE, eval = TRUE, message = FALSE, warning = FALSE}
x10 <- data.frame(arvore = paste0("A", 1:5),
...
basal_area <- function(dbh, is_cm = TRUE) {
  ...
  # Calcula a área basal de cada árvore (π * (dbh/2)^2)
...
```{r}
(var_amo <- sum(quadrado_desvio(x)) / 9)
```
...
map_df(df, mean) # retorna um data frame
```
